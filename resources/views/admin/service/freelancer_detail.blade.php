        ...
        <div class="col-xl-4 col-md-6 mb-30">
            <div class="card b-radius--10 box--shadow1 overflow-hidden">
                <div class="card-header">
                    <h6>@lang('Freelancer Information')</h6>
                </div>
                <div class="card-body">
                    <ul class="list-group">
                        <li class="list-group-item d-flex justify-content-between align-items-center">
                            @lang('Fullname')
                            <span class="fw-bold">
                                <a href="{{ route('admin.freelancers.detail', $service->freelancer_id) }}">{{ __(@$service->freelancer->fullname) }}</a>
                            </span>
                        </li>
                        <li class="list-group-item d-flex justify-content-between align-items-center">
                            @lang('Username')
                            <span class="fw-bold">
                                <a href="{{ route('admin.freelancers.detail', $service->freelancer_id) }}">{{ __(@$service->freelancer->username) }}</a>
                            </span>
                        </li>
                        <li class="list-group-item d-flex justify-content-between align-items-center">
                            @lang('Email')
                            <span class="fw-bold">{{ __(@$service->freelancer->email) }}</span>
                        </li>
                        <li class="list-group-item d-flex justify-content-between align-items-center">
                            @lang('Mobile')
                            <span class="fw-bold">{{ __(@$service->freelancer->mobile) }}</span>
                        </li>
                        <li class="list-group-item d-flex justify-content-between align-items-center">
                            @lang('Country')
                            <span class="fw-bold">{{ __(@$service->freelancer->address->country) }}</span>
                        </li>
                        <li class="list-group-item d-flex justify-content-between align-items-center">
                            @lang('Balance')
                            <span class="fw-bold">{{ showAmount(@$service->freelancer->balance) }} {{ $general->cur_text }}</span>
                        </li>
                        <li class="list-group-item d-flex justify-content-between align-items-center">
                            @lang('Created_at')
    ...
    <script>
        (function($) {
            ...
            $('.statusBtn').on('click', function() {
                ...
                modal.find('form').attr('action', `{{ route('admin.freelancer.service.status', '') }}/${$(this).data('id')}`);
