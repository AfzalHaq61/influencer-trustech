                                ...
                                <div class="chat__msg" id="coversation_body">
                                    ...
                                    <div class="chat__msg-header border-bottom">
                                        <div class="post__creator align-items-center d-flex flex-wrap justify-content-between">
                                            <div class="post__creator-content mb-3">
                                                <h5 class="name">{{ __(@$user->username) }}</h5>
                                                @if ($user->status == 0)
                                    ...
                                    <div class="chat__msg-body position-relative">
                                        ...
                                        <ul class="msg__wrapper mt-3" id="message">
                                            @if ($user)
                                                @include($activeTemplate . 'freelancer.conversation.message')
                                            @endif
                                        </ul>
                                    </div>
                                    ...
                                    <div class="chat__msg-footer mt-0">
                                        ...
                                        <form action="" method="POST" class="send__msg" enctype="multipart/form-data" id="messageForm">
                                            <div class="d-flex gap-2">
                                                <textarea type="text" class="form-control form--control messageVal" name="message" contenteditable="true" placeholder="@lang('Write Message')..." required>@if($message != null){{$message}}@endif</textarea>
...
@push('script')
    <script>
        (function($) {
            ...
            //Job quereis automatic message sending
            if(`$message` != null){
                var jobOrderMessage = new FormData($("#messageForm")[0]);
                $.ajax({
                    headers: {
                        "X-CSRF-TOKEN": "{{ csrf_token() }}",
                    },
                    url: "{{ route('freelancer.conversation.store', @$conversation->id) }}",
                    method: "POST",
                    data: jobOrderMessage,
                    async: false,
                    processData: false,
                    contentType: false,
            ...
            function loadMore(messageCount){
                ...
                $.ajax({
                    ...
                    url: "{{ route('freelancer.conversation.message')}}",
                    success: function (response) {
