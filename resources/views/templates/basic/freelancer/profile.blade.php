...
<div class="pt-50 pb-50">
    <div class="influencer-profile-area container">
        <div class="container-fluid px-3">
            @if($freelancer->home_screen_photos != null && $freelancer->home_screen_videos != null)
                @php
                    $photos = json_decode($freelancer->home_screen_photos);
                    $videos = json_decode($freelancer->home_screen_videos);
                @endphp
                <div class="w-100 row">
                    <div class="collage gap-2 collage-5" data-controller="glightbox">
                        @foreach($photos AS $photo)
                        <a href="javascript:void(0)" class="lightBoxImageOpen" data-image="{{ url('/') }}/core/public/sample/{{$freelancer->username}}/photos/{{$photo}}">
                            <img src="{{ url('/') }}/core/public/sample/{{$freelancer->username}}/photos/{{$photo}}" class="img-fluid" />
                        </a>
                        @endforeach
                    
                        @foreach($videos AS $video)
                        <a href="javascript:void(0)" class="lightBoxVideoOpen" data-video="{{ url('/') }}/core/public/sample/{{$freelancer->username}}/videos/{{$video}}">
            ...
            <div class="influencer-profile-wrapper">
                    <div class="row">
                        <div class="col-md-8">
                            <div class="content">
                                <h4 class="fw-medium name account-status d-inline-block">{{ $freelancer->fullname }}: {{ $freelancer->about_me }}</h4><br>
                                <span class="text--base"> {{ $freelancer->profession }}</span>
                                <span class="w-100"><p><i> {{ $freelancer->summary }}</i></p></span>
                                @if($freelancer->categories)
                            ...
                            <div class="d-flex justify-content-between mt-4 mb-2">
                                <h6 class="fw-medium">@lang('About') @ {{ $freelancer->username }}</h6>
                                {{-- <a href="#"><i class="fa fa-solid fa-signal"></i>&nbsp; See More Analytics</a> --}}
                            </div>
                            @if(count($freelancer->socialLink) != 0)
                                @php
                                    $followers=[];
                                @endphp
                                @foreach ($freelancer->socialLink as $social)
                                    @php
                            ...
                            <div class="d-flex justify-content-center mt-5">
                                <h4 id="services" data-services="0">>@lang('This Freelancer has not provided any services yet')</h4>
                            </div>
                            @else
                            <h6 class="mt-3">@lang('Services')</h6>
                            <p  class="mb-3">@lang('The specific deliverables available from this Seller. Choose one.')</p>
                        ...
                        <div class="col-md-4">
                            <div>
                                <img src="{{ url('/') }}/core/public/sample/{{$freelancer->username}}/photos/{{$freelancer->image}}" class="rounded-circle" width="70" height="70" />
                                <a href="#"><strong class="text--base ml-2">{{ $freelancer->fullname }}</strong></a>
                                @foreach ($freelancer->socialLink as $social)
                                @php
                                    $value = $social->followers;
                            ...
                            <div class="w-100 card mt-2">
                                <div class="card-body text-bg-primary">
                                    ...
                                    <div class="pt-2">
                                        <a href="{{ route('user.conversation.freelancer.create', $freelancer->id) }}" class="msgButton">@lang('Message')</a>
                                            ...
                                            <div class="row mb-3 text-gray-600 gx-3 gy-2 font-size-base-to-mobile-sm">
                                                ...
                                                @if(!authFreelancerId())
                                                    ...
                                                    <div class="d-flex justify-content-center">
                                                        <a href="{{ route('user.order.freelancer.create-offer', $freelancer->username) }}" class="text--base">
            ...
            <div class="row position-relative">
                <div class="d-flex justify-content-start pt-4 pb-2 mx-2">
                    <h4>@lang('Similar Freelancers')</h4>
                </div>
    ...
    <script>
        (function($) {
            ...
            if(services !=0){
                ...
                $('.buyNowRoute').attr('href', '/influencer/client/order/freelancer-service/'+serviceId);
            }
