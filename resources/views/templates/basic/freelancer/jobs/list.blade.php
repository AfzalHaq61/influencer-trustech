...
<div class="d-flex justify-content-between align-items-center position-relative mb-4 flex-wrap gap-4">
    ...
    <div class="d-flex align-items-start flex-wrap gap-2">
        <a class="btn btn--outline-custom {{ menuActive('freelancer.jobs.all') }}" aria-current="page" href="{{ route('freelancer.jobs.all') }}">@lang('All')</a>
        <a class="btn btn--outline-custom {{ menuActive('freelancer.jobs.pending') }}" href="{{ route('freelancer.jobs.pending') }}">@lang('Pending')</a>
        <a class="btn btn--outline-custom {{ menuActive('freelancer.jobs.approved') }}" href="{{ route('freelancer.jobs.approved') }}">@lang('Approved')</a>
        <a class="btn btn--outline-custom {{ menuActive('freelancer.jobs.cancelled') }}" href="{{ route('freelancer.jobs.cancelled') }}">@lang('Cancelled')</a>
        <a class="btn btn--outline-custom {{ menuActive('freelancer.jobs.closed') }}" href="{{ route('freelancer.jobs.closed') }}">@lang('Closed')</a>
    </div>
                ...
                <tr>
                    ...
                    <td data-label="@lang('Action')">
                        <div class="d-flex flex-wrap gap-2 justify-content-end">
                            @if($job->status == 0 )
                                <a href="{{ route('freelancer.jobs.edit', $job->id) }}" class="btn btn--sm btn--outline-base @if ($job->status == 2) disabled @endif">
                                    <i class="la la-edit"></i> @lang('Edit')
                                </a>
                            @endif
                            @if($job->status == 2 || $job->status == 3 || $job->status == 4 || $job->status == 7)
                                <a href="{{ route('freelancer.jobs.detail', $job->id) }}" class="btn btn--sm btn--outline-base">
