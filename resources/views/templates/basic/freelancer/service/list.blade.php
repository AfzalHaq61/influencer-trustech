...
<div class="d-flex justify-content-between align-items-center position-relative mb-4 flex-wrap gap-4">
    ...
    <div class="d-flex align-items-start flex-wrap gap-2">
        <a class="btn btn--outline-custom {{ menuActive('freelancer.service.all') }}" aria-current="page" href="{{ route('freelancer.service.all') }}">@lang('All')</a>
        <a class="btn btn--outline-custom {{ menuActive('freelancer.service.pending') }}" href="{{ route('freelancer.service.pending') }}">@lang('Pending')</a>
        <a class="btn btn--outline-custom {{ menuActive('freelancer.service.approved') }}" href="{{ route('freelancer.service.approved') }}">@lang('Approved')</a>
        <a class="btn btn--outline-custom {{ menuActive('freelancer.service.rejected') }}" href="{{ route('freelancer.service.rejected') }}">@lang('Rejected')</a>
    </div>
        ...
        <table class="table--responsive--lg table">
            <thead>
                <tr>
                    ...
                    <th>@lang('Order')</th>
                    @if(request()->routeIs('freelancer.service.all'))
                    <th>@lang('Status')</th>
                    @endif
                    <th>@lang('Action')</th>
                </tr>
                ...
                <tr>
                    ...
                    <td data-label="@lang('Action')">
                        <div class="d-flex flex-wrap gap-2 justify-content-end">
                            <a href="{{ route('freelancer.service.edit', $service->id) }}" class="btn btn--sm btn--outline-base @if ($service->status == 2) disabled @endif">
                                <i class="la la-edit"></i> @lang('Edit')
                            </a>

                            <a href="{{ route('freelancer.service.orders', $service->id) }}" class="btn btn--sm btn--outline-info @if ($service->status != 1) disabled @endif">
