...
<form action="" class="d-flex flex-wrap justify-content-end ms-auto table--form mb-3">
    <div class="input-group">
        <input type="text" name="search" class="form-control form--control" value="{{ request()->search }}" placeholder="@lang('Order No / freelancer')">
    ...
    <tbody>
        ...
        <tr>
            ...
            <td data-label="@lang('freelancer')">
                <span class="fw-bold"><a href="{{ route('freelancer.profile', [slug($order->freelancer->username), $order->freelancer_id]) }}" class="text--base">{{ __(@$order->freelancer->username) }}</span>
            </td>
            ...
            <td data-label="@lang('Action')">
                <div class="d-flex flex-wrap gap-1 justify-content-end">
                    ...
                    @endif
                    <a href="{{ route('user.order.freelancer.detail',$order->id) }}" class="btn btn--sm btn--outline-base">
                        <i class="la la-desktop"></i> @lang('Detail')
                    </a>
                    <a href="{{ route('user.order.freelancer.conversation.view',$order->id) }}" class="btn btn--sm btn--outline-info">
