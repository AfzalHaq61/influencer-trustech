...
@section('content')
    ...
    $registerFreelancer = getContent('freelancer_register.content', true);
                ...
                <div class="row gy-5 align-items-center">
                    <div class="col-lg-6">
                        <div class="account-thumb-wrapper">
                            <img src="{{ getImage('assets/images/frontend/user_register/' . @$register->data_values->image, '660x450') }}" class="mw-100 h-100">
                        </div>
                    </div>
                    <div class="col-lg-6">
                        <div class="account-content">
                            <div class="d-flex justify-content-between flex-wrap gap-1 pb-5">
                                <div class="account-content-left">
                                    <h3 class="this-page-title">@lang($register->data_values->title)</h3>
                                </div>
                            ...
                            <div class="text-center">
                                <p class="mt-3">@lang('Have an account?')
                                    <a href="{{ route('user.login') }}" class="text--base login-url">@lang('Login here')</a>
                                </p>
                            </div>
                        </div>
                    </div>
...
@push('script')
    <script>
        ...
        (function($) {
            ...
            let loginUrl;
            let pageTitle;
            let session = sessionStorage.getItem("userTypeRedg");
            if(session =='client'){
                ...
                action = `{{ route('freelancer.register') }}`;
                loginUrl = `{{ route('freelancer.login') }}`;
                pageTitle = `{{ __(@$registerFreelancer->data_values->title) }}`;
            ...
            $('.actionBtn').on('click', function() {
                ...
                if ($(this).data('type') == 'client') {
                    ...
                    loginUrl = `{{ route('user.login') }}`;
                    pageTitle = `{{ __(@$register->data_values->title) }}`;
                    sessionStorage.setItem("userTypeRedg","client");

                }else if($(this).data('type') =='freelancer'){
                    action = `{{ route('freelancer.register') }}`;
                    loginUrl = `{{ route('freelancer.login') }}`;
                    pageTitle = `{{ __(@$registerFreelancer->data_values->title) }}`;
                    sessionStorage.setItem("userTypeRedg","freelancer");
                }else {
                    action = `{{ route('influencer.register') }}`;
                    loginUrl = `{{ route('influencer.login') }}`;
                    pageTitle = `{{ __(@$registerInfluencer->data_values->title) }}`;
                    sessionStorage.setItem("userTypeRedg","influencer");
                }
                $('form')[0].action = action;
