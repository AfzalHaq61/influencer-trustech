...
class ManageJobController extends Controller
{
    ...
    protected function jobData($scope = null)
    {
        ...
        $jobs = $jobs->searchable(['name', 'user:username', 'influencer:username', 'freelancer:username', 'jobCategory:name', 'JobSubCategory:name'])->filter(['user_id','influencer_id','freelancer_id'])->latest()->with(['user', 'jobCategory', 'JobSubCategory','influencer','freelancer'])->paginate(getPaginate());
