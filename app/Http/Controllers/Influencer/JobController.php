...
class JobController extends Controller
{
    ...
    public function jobCreate()
    {
        ...
        return view($this->activeTemplate . 'influencer.jobs.create', compact('pageTitle', 'jobCategories','jobSkills'));
    }
    ...
    public function jobInsert(Request $request, $id = 0){
        ...
        if ($id) {
            $job          = Job::where('id', $id)->where('influencer_id',authInfluencerId())->firstOrFail();
            $notification = 'Job updated successfully';
        } else {
            $job          = new Job();
            $job->influencer_id = authInfluencerId();
    ...
    public function jobDetail($id){
        ...
        return view($this->activeTemplate . 'influencer.jobs.job_detail', compact('pageTitle', 'order'));
    }
    ...
    public function completeStatusInfluencer($id){
        ...
        $loggedIninfluencer = authInfluencer();
    ...
    public function allJobs($scope = null){
        ...
        $jobs = $jobs->where('influencer_id', authInfluencerId())->latest()->with(['influencer', 'jobCategory', 'JobSubCategory'])->paginate(getPaginate());

        return view($this->activeTemplate . 'influencer.jobs.list', compact('pageTitle', 'jobs'));
    }
    ...
    public function editJob(Job $job){
        ...
        return view($this->activeTemplate . 'influencer.jobs.edit_job', compact('pageTitle', 'job', 'jobCategories'));
    }
    ...
    protected function filterJobs($status, $title){
        ...
        $pageTitle = $title;
        $jobs =Job::where('influencer_id', authInfluencerId())->where('status', $status)->latest()->with(['influencer', 'jobCategory', 'JobSubCategory'])->paginate(getPaginate());
        return view($this->activeTemplate . 'influencer.jobs.list', compact('pageTitle', 'jobs'));
    }
    ...
    public function jobApplications(){
        ...
        $jobs =Job::where('influencer_id', authInfluencerId())->where('status', 1)->latest()->with(['influencer', 'jobCategory', 'JobSubCategory','jobApplications'])->paginate(getPaginate());
        return view($this->activeTemplate . 'influencer.jobs.job_applications', compact('pageTitle', 'jobs'));
    }
    ...
    public function jobApplicants($id){
        ...
        return view($this->activeTemplate . 'influencer.jobs.applicants', compact('pageTitle', 'job_applications'));
    }

    public function jobAssignToYou (){
       $jobs = ApprovedJobOrder::where('influencer_id', authInfluencerId())->paginate(getPaginate());
       $pageTitle = 'Assign to you';
       return view($this->activeTemplate . 'influencer.jobs.assign_to_you', compact('pageTitle', 'jobs'));
    }

    public function jobOrderDetail($order_id){
        $order = ApprovedJobOrder::find($order_id);
        $pageTitle = 'Job Order Detail';
        return view($this->activeTemplate . 'influencer.jobs.job_order_detail', compact('pageTitle', 'order'));
    }
    ...
    public function assignJob($user_type, $user_id, $application_id){
        ...
        if($user_type == 'influencer'){
            return redirect()->route('influencer.influencer.payment');

        }elseif($user_type == 'freelancer'){
            return redirect()->route('influencer.freelancer.payment');

        }elseif(($user_type == 'user')){
            return redirect()->route('influencer.client.payment');
        }
